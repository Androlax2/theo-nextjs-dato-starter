...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!process.env.DATOCMS_CMA_TOKEN) {
      return invalidRequestResponse("Missing required environment variables");
    }
    ...
    const websitePath = await recordToWebsiteRoute(
      item,
      itemTypeApiKey as ItemTypeApiKey,
      locale as Locale,
    );

    const slug = await recordToSlug(
      item,
      itemTypeApiKey as ItemTypeApiKey,
      locale as Locale,
    );
