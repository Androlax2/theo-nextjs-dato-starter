...
export async function executeQuery<Result, Variables>(
  ...
  if (!process.env.DATOCMS_DRAFT_CONTENT_CDA_TOKEN) {
    throw new Error(
      "DATOCMS_DRAFT_CONTENT_CDA_TOKEN environment variable is not set",
    );
  }

  if (!process.env.DATOCMS_PUBLISHED_CONTENT_CDA_TOKEN) {
    throw new Error(
      "DATOCMS_PUBLISHED_CONTENT_CDA_TOKEN environment variable is not set",
    );
  }
